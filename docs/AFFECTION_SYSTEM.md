...
所有卡通用的加减分底座，位于 `backend/services/character_state.py`：
...
**修改方式**：直接编辑 `backend/services/character_state.py` 中 `_AFFECTION_BASE_RULES` 字典。
...
**修改方式**：修改 `backend/services/character_state.py` 中 `_DAILY_AFFECTION_CAP = 15`。
...
编辑 `backend/services/character_state.py` 里的 `_AFFECTION_BASE_RULES` 字典即可，改后重启后端生效。

### 调整某个事件的冷却时间

编辑 `backend/services/character_state.py` 里的 `_AFFECTION_COOLDOWN_SECONDS` 字典。

### 调整日上限

修改 `backend/services/character_state.py` 里的 `_DAILY_AFFECTION_CAP = 15`。

### 调整边际递减系数

修改 `backend/services/character_state.py` 里的 `_AFFECTION_DIMINISHING_RETURNS` 列表（从第 1 次到第 N 次的系数）。

### 调整阶段阈值

修改 `backend/services/character_state.py` 里的 `_PHASE_THRESHOLDS` 字典。
