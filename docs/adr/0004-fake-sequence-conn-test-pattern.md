...
aifriend 的 大部分测试文件中，大部分单元测试需要隔离数据库。传统的 mock 方式（unittest.mock、pytest-mock）有两个问题：
...
- **方案 A：真实测试数据库（test DB）** — 每个测试连接真实的 PostgreSQL 测试库
  - 未选原因：速度太慢（每个测试需要 setup/teardown 数据），不适合 大部分测试的快速反馈。但集成测试（tests/integration/）使用真实数据库
