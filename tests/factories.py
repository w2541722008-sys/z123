...
所有工厂方法接受 **overrides 来自定义字段，返回 dict 或 FakeRow。
用法：
    from tests.factories import UserFactory, CharacterFactory

    user = UserFactory.build(id=1, plan_type="vip")
    row = UserFactory.as_row(email="admin@test.com")
"""
...
class MessageFactory:
    ...
    def history(cls, turns: int = 3, character_id: str = "char-001", user_id: int = 1) -> list[dict]:
        ...
        for i in range(1, turns + 1):
            msgs.append(cls.build(
                id=i * 2 - 1, user_id=user_id, character_id=character_id,
                role="user", content=f"User message {i}",
            ))
            msgs.append(cls.build(
                id=i * 2, user_id=user_id, character_id=character_id,
    ...
    def history_rows(cls, turns: int = 3, character_id: str = "char-001", user_id: int = 1) -> list[FakeRow]:
        return [FakeRow(m) for m in cls.history(turns, character_id, user_id)]
...
class CharacterStateFactory:
    ...
    def with_cooldown(cls, event_name: str, timestamp: str | None = None, **overrides) -> dict:
        ...
        return cls.build(
            _last_event_timestamps={event_name: ts},
            **overrides,
        )
    ...
    @classmethod
    def with_diminishing(self, event_name: str, count: int, **overrides) -> dict:
        return self.build(
            _daily_event_counts={event_name: count},
            **overrides,
        )
