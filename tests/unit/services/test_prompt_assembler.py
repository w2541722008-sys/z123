...
class TestRuntimeLayerHelpers:
    ...
    def test_select_mode_builder_prefers_card_type_over_asset_type(self):
        ...
        # card_type 是玩法总开关：对话陪伴不应被导卡层 asset_type 带进剧情沙盒 builder
        assert builder.__name__ == "_build_character_mode_messages"

        builder = _select_mode_builder("intimate", "unknown")
        assert builder.__name__ == "_build_character_mode_messages"

    def test_intimate_scenario_asset_includes_life_profile_in_prompt(self):
        messages = build_layered_chat_messages_from_context(
            PromptBuildContext(
                character={
                    ...
                    "card_type": "intimate",
                    "asset_type": "scenario",
                    "life_profile_json": '{"basic_info":"林深从小在茶馆长大"}',
                },
        ...
        assert "【角色人生档案】" in system_text
        assert "林深从小在茶馆长大" in system_text
