...
def admin_create_character(
    ...
    if not char_id:
        raise BadRequestError(detail="角色ID不能为空")
    if not name:
        raise BadRequestError(detail="角色名不能为空")
    if not system_prompt:
        raise BadRequestError(detail="主指令（System Prompt）不能为空")

    if not char_id.replace("_", "").isalnum():
        raise BadRequestError(detail="角色ID只能包含英文、数字和下划线")

    def _do_create():
        if char_repo.check_character_exists(conn, char_id):
            raise ConflictError(detail=f"角色ID '{char_id}' 已存在")
        ...
        try:
            ...
            if not isinstance(tags_list, list):
                raise BadRequestError(detail="tags必须是JSON数组格式")
        except json.JSONDecodeError:
            raise BadRequestError(detail="tags格式错误，必须是有效的JSON")
        ...
        if required_plan not in valid_plans:
            raise BadRequestError(detail=f"required_plan必须是以下之一: {', '.join(valid_plans)}")

        if not (0 <= home_priority <= 9999):
            raise BadRequestError(detail="home_priority必须在0-9999之间")
...
def admin_update_character(
    ...
    if not safe_direct and not rl_updates:
        raise BadRequestError(detail="没有合法的可更新字段")
            ...
            if rules_raw and str(rules_raw).strip():
                try:
                    ...
                    if not isinstance(rules, dict):
                        raise BadRequestError(detail="affection_rules_json 必须是 JSON 对象")
